...
char * LdapServiceGetQuery(char * field, char * value) {
	...
	int bufferSize = sizeof(char) * (strlen(value) + strlen(field) + strlen(
			querySeparator));
	char * query = calloc(bufferSize, sizeof(char));

	strcat(query, field);
	strcat(query, querySeparator);
	strcat(query, value);

	printf("LOG: Se prepara la query LDAP siguiente: %s\n\n", query);
...
AbstractNotice LdapServiceGetNotice(PLDAP_RECORD_OP recordOp,
		PLDAP_RECORD record) {
	char * id = calloc(500, sizeof(char));
	char * body = calloc(5000, sizeof(char));
	char * header = calloc(3000, sizeof(char));
	char * groupName = calloc(500, sizeof(char));
	...
	printf(
			"Log: Se trajo de OPEN DS la siguiente la noticia con ID = %s y HEADER = %s y GROUPNAME = %s\n",
			id, header, groupName);
